...
A developer revision never reuses a rejected mutable workspace. After failure/recovery, create a fresh isolated candidate, call the kernel's checked `replace_workspace` with the expected old ID and reason, then clean the old worktree. The event trail must retain both identities.
